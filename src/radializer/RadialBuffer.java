...
public class RadialBuffer {
	...
	public void update() {
		...
		if (columnCounter < inputImage.width) {
			...
			// PERO DIBUJANDOLA RADIALMENTE DE ABAJO HACIA ARRIBA (SALE
			// ESPEJADA)
	...
	public void updateUsingVertex() {
		...
		/*
		 * imageBuffer.beginShape(p5.QUADS); imageBuffer.texture(inputImage);
		 * imageBuffer.vertex(100,100,0,0); // 0 imageBuffer.vertex(500,100,
		 * 1,0); // 1 imageBuffer.vertex(p5.mouseX, p5.mouseY, 1, 1); // 2
		 * Esquina imageBuffer.vertex(100, 400, 0, 1); // 3
		 * imageBuffer.endShape();
		 */
